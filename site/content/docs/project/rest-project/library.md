...
* List All Users
* List All Books
* List Books by Search Criteria

These are not the actual APIs, they are the descriptive names of the functionality provided by the APIs. Remember that RESTful APIs operate on resources. In the case of Library data, the resource is the one of the tables in your Library DB. The APIs, will be various operations on the table(s). Depending on your design, the exposed resource/ API may use other tables under the hood.

For example, **List All Users** could look like `/users` and would return all the the rows and columns from the users table. Users should have **at least** a name, userID and any other relevant data for a user.

**List All Books** could look like `/books` and would return all data related to books, including which library the book is in. This could be from one table, or multiple tables (depending on how you set up your DB). It is up to your work figure out the appropriate way. You may even decide to modify your schema as you investigate how to implement the API.

Finally, **List Books by Search Criteria** (e.g. fiction or non-fiction books, or another search factor) would return details on books based on search criteria. The API could search by type and could either be `/books/{type}`, where `type` would be the type of the desired book (`fiction` or `non-fiction`) and is part of the URL, or could be `/books?type={book_type}`, where `book_type` is the type of the book you want, but the information is sent as a query string for the API. You could search by author, and use the author name as a search criteria, and so on. Keep in mind that this is dependent on the schema and table relationships you have set up, so your API might look quite different.
...
* Similarly, when calling the API for **List All Books** you should get a fixed (correct) number of rows with the appropriate columns.
* If you call the **List Books by Search Criteria** API, your unit test should confirm that the data returned is what you would expect (again, you will need to work that out ahead of time).
* You should do multiple searches, first with just a single search term, and then another with multiple search terms. If you do a search with a title or author that does not exist, you should get an empty list back; otherwise you get the book(s) along with which library and how many are available.
...
4. List all checkouts for a given user. This is a `GET`. Your choice on parameters
5. A user can login (see below) to the library DB. Note that the login functionality is needed for the checkout and reserve functions.
6. A logged in user can checkout or reserve a book. Consider where to place parameters.
...
* You can add a new user with a password and any other user information
    * Creating a user could be done either by logging in first with an ‘admin’ account (and then creating the user), or by assuming that creating a user does not require pre-authentication. Your choice.
...
* You can edit a users information; if you try to edit a non-existent user, the API fails (users can only edit their own data)
* You can remove a user; again, if the user doesn’t exist, the API fails (users can only remove their own account)
* If you try to remove a user (who exists), and don’t have the correct authentication session key, the API fails
* You can list all books, or books by title or type. Make sure you have the correct data in the DB to test this properly. Anyone can do this.
* You can perform a new checkout of a book by title. Only an authenticated user can do this. If no user is authenticated, an error is returned.
